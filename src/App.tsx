...
import { codeReducer, initialState, RhythmType } from './state/codeState';
import { getNextShockableNode } from './data/algorithm';
...
const App: React.FC = () => {
  ...
  const handleNodeTap = (nodeId: string) => dispatch({ type: 'JUMP_NODE', payload: { nodeId } });
  ...
  const handleStartCode = (mode: any, startNodeId: string, rhythm: RhythmType) => {
      ...
      <div className="h-[12vh] md:h-[10vh] border-b border-gray-800 bg-black flex flex-col md:flex-row items-center justify-between px-2 md:px-6 shrink-0 z-30 shadow-md">
        ...
        <div className="w-full flex items-center justify-between md:w-auto">
          <div className="flex items-center gap-2 md:gap-4">
            <h2 className="text-xl md:text-3xl font-bold tracking-widest uppercase" style={{ color: 'var(--color-neon-green)' }}>CodeVector</h2>
          </div>
        ...
        {/* CPR TIMER - Massive on Mobile */}
        <div className="w-full md:w-auto flex items-center justify-center gap-3 bg-black mt-2 md:mt-0 px-2 md:px-4 py-2 rounded-xl border border-gray-800 shadow-inner">
          <span className="uppercase tracking-widest text-xs md:text-sm font-semibold text-gray-400">CPR</span>
          <span className={`text-4xl md:text-4xl font-mono font-bold ${state.cprTimerSecondsRemaining === 0 ? 'text-red-500 animate-pulse' : ''}`}
          ...
          <div className="flex justify-center ml-2 md:ml-4 gap-2">
            <button onClick={() => dispatch({ type: 'RESET_CPR_TIMER' })} disabled={state.codeEnded} className="min-h-[44px] px-3 py-1 bg-gray-900 border-[1px] border-gray-800 hover:bg-gray-800 text-gray-300 rounded text-xs font-bold transition-all">RESET</button>
            {state.cprTimerSecondsRemaining === 0 && (
              <button onClick={() => dispatch({ type: 'COMPLETED_CPR_CYCLE' })} disabled={state.codeEnded} className="min-h-[44px] px-3 py-1 bg-blue-900 border border-blue-500 hover:bg-blue-800 text-blue-200 rounded text-xs font-bold transition-all animate-pulse">CONTINUE</button>
            )}
          </div>
        </div>
